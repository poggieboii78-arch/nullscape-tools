...
export default function Home(){
  const {run,update:updateRun,reset:resetShared,applyLinkedRun}=useSharedRun(storageKey);const{level,players,party,difficulty,gifts}=run;
  const [mode,setMode]=useState<Mode>("protection");const [ready,setReady]=useState(false);
  const quickLinkState=useMemo(()=>({mode}),[mode]);
  const applyQuickLinkState=useCallback((state:Record<string,unknown>)=>{if(state.mode==="protection"||state.mode==="purification"||state.mode==="chance")setMode(state.mode);},[]);
  ...
  return <main>
    ...
    <RunDock run={run} update={updateRun} reset={reset} applyLinkedRun={applyLinkedRun} toolId="altar" quickLinkState={quickLinkState} applyQuickLinkState={applyQuickLinkState} toolSteps={[{selector:".mode-tabs",title:"Choose what you want to calculate",text:"Use these tabs to check a Protection price, plan a Purification order, or see the possible Chance outcomes."},{selector:"[data-tour='altar-result'] .result-copy",title:"The answer changes with your run",text:"Update your level, players, Gifts, or other run information in the Quick Menu. The result here updates automatically."}]}/>
  </main>;
}
