...
export function useSharedRun(legacyKey:string){
  ...
  const update=useCallback((change:Partial<SharedRun>|((current:SharedRun)=>Partial<SharedRun>))=>setRun(current=>{let patch=typeof change==="function"?change(current):change;const nextParty=patch.party??current.party;if((current.party==="solo"||current.party==="duo")&&(nextParty==="party"||nextParty==="party-plus")&&nextParty!==current.party){const upgrades={...current.upgrades,...(patch.upgrades??{})};const paycheck=upgrades.Paycheck??0;if(paycheck>1)upgrades.Paycheck=paycheck-1;else delete upgrades.Paycheck;patch={...patch,upgrades};}const next=normalize({...current,...patch,updatedAt:Math.max(Date.now(),current.updatedAt+1)});recordActivity(current,next);latest.current=next.updatedAt;persist(next);return next;}),[]);
