...
export function RunDock({run,update,reset,applyLinkedRun,toolId,toolSteps=[]}:{run:SharedRun;update:(change:Partial<SharedRun>|((current:SharedRun)=>Partial<SharedRun>))=>void;reset:()=>void;applyLinkedRun:(run:SharedRun)=>void;toolId:string;toolSteps?:TourStep[]}){
  ...
  const steps=useMemo<TourStep[]>(()=>[
    ...(toolId==="home"?[
      ...
      {selector:"[data-tour='calculator-link']",title:"Share the run with QuickLink",text:"Send an editor invite to people in the lobby. Send the view-only spectator link to everyone else so they can watch without changing the run or player count."},
  ...
  useEffect(()=>{if(steps.length>0&&localStorage.getItem(seenKey)!=="1")setTour(0);},[seenKey,steps.length]);
  ...
  return <>
    ...
    {tour>=0&&steps[tour]&&<div className="tour-layer">{focusRect&&<div className="tour-spotlight" style={focusRect}/>}<section className="tour-card" role="dialog" aria-modal="false" aria-label="Website walkthrough"><button className="tour-close" onClick={finish} aria-label="Close walkthrough">×</button><small>{tour+1} / {steps.length}</small><h2>{steps[tour].title}</h2><p>{steps[tour].text}</p><div><button onClick={finish}>Skip</button><span>{steps.map((_,i)=><i key={i} className={i===tour?"active":""}/>)}</span><button className="tour-next" onClick={()=>tour===steps.length-1?finish():setTour(tour+1)}>{tour===steps.length-1?"Done":"Next →"}</button></div></section></div>}
  </>;
}
