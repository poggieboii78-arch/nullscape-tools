...
function gradientColors(from:string,to:string,count:number){const a=hexToRgb(safeColor(from)),b=hexToRgb(safeColor(to));return Array.from({length:Math.max(1,count)},(_,i)=>{const t=count<=1?0:i/(count-1);return rgbToHex(a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t,a[2]+(b[2]-a[2])*t);});}
