...
import type { BlockType, CompendiumBlock, CompendiumClass, CompendiumData, CompendiumTech } from "./types";
...
export default function Editor() {
    ...
    fetch("/api/editor", { cache: "no-store" }).then(readJson).then((payload) => {
      ...
      try {
        ...
        if (stored) {
          ...
          setData(next); setClassId(next.classes[0]?.id ?? ""); setTechId(next.classes[0]?.techs[0]?.id ?? "");
  ...
  const activeTech = activeClass?.techs.find((item) => item.id === techId);
  ...
  function chooseClass(id: string) {
    const next = data.classes.find((item) => item.id === id); setClassId(id); setTechId(next?.techs[0]?.id ?? ""); setClassPanel(true);
  }
  function addClass() {
  ...
  function deleteClass() {
    ...
    const classes = data.classes.filter((item) => item.id !== activeClass.id); dirty({ ...data, classes }); setClassId(classes[0]?.id ?? ""); setTechId(classes[0]?.techs[0]?.id ?? "");
  }
  function addTech() {
  ...
  function deleteTech() {
    ...
    const techs = activeClass.techs.filter((item) => item.id !== activeTech.id); replaceClass({ ...activeClass, techs }); setTechId(techs[0]?.id ?? "");
  }
  ...
  return <main className="editor-shell">
    ...
    <div className="editor-grid">
      <aside className="class-column">
        ...
        <div className="structure-list">{data.classes.map((item) => <button key={item.id} className={item.id === classId ? "active" : ""} onClick={() => chooseClass(item.id)}><span style={{ background: item.accent }}>{iconPreview(item.icon, "✦")}</span><div><b>{item.name}</b><small>{item.techs.length} tech{item.techs.length === 1 ? "" : "s"}</small></div><i>{item.published ? "" : "Draft"}</i></button>)}</div>
      ...
      <aside className="tech-column">
        <div className="column-heading"><div><small>{activeClass?.name ?? "Choose a class"}</small><h2>Techs</h2></div><button onClick={addTech} disabled={!activeClass} aria-label="Add tech">+</button></div>
        ...
        <div className="structure-list tech-structure">{activeClass?.techs.map((item, index) => <button key={item.id} className={!classPanel && item.id === techId ? "active" : ""} onClick={() => { setTechId(item.id); setClassPanel(false); }}><span>{iconPreview(item.icon, String(index + 1).padStart(2, "0"))}</span><div><b>{item.title}</b><small>{item.summary || "No summary yet"}</small></div><i>{item.published ? "" : "Draft"}</i></button>)}</div>
        {activeTech && !classPanel && <div className="reorder-row"><button onClick={() => replaceClass({ ...activeClass!, techs: move(activeClass!.techs, activeClass!.techs.findIndex((i) => i.id === techId), activeClass!.techs.findIndex((i) => i.id === techId) - 1) })}>↑ Move</button><button onClick={() => replaceClass({ ...activeClass!, techs: move(activeClass!.techs, activeClass!.techs.findIndex((i) => i.id === techId), activeClass!.techs.findIndex((i) => i.id === techId) + 1) })}>↓ Move</button></div>}
      </aside>
      ...
      <section className="edit-canvas">
        {!activeClass ? <div className="editor-empty"><span>✦</span><h1>Create your first class</h1><p>Classes become the tabs across the top of the compendium.</p><button onClick={addClass}>Add class</button></div> : classPanel ? <div className="form-page">
          ...
          <div className="block-list">{activeTech.blocks.map((block, index) => <div className="block-card" key={block.id}>
            ...
            {block.type === "metronome" && <div className="metronome-fields">
              <label><span>BPM</span><input type="number" min="20" max="300" value={block.bpm ?? 90} onChange={(e) => updateBlock(block.id, { bpm: Math.max(20, Math.min(300, Number(e.target.value) || 90)) })} /></label>
              <label><span>Count-in beats</span><select value={block.countIn ?? 4} onChange={(e) => updateBlock(block.id, { countIn: Number(e.target.value) })}><option value="0">None</option><option value="2">2 beats</option><option value="4">4 beats</option><option value="8">8 beats</option></select></label>
              ...
              <label className="wide"><span>One beat per line — use + together, or put a millisecond delay between inputs</span><textarea rows={7} value={block.content} placeholder={'Shift + W\nM1 (10ms) Space\nQ > 35ms > M1\nWait'} onChange={(e) => updateBlock(block.id, { content: e.target.value })} /></label>
              ...
              <p className="key-help wide">Examples: <kbd>Q + M1</kbd> presses both together. <kbd>M1 (10ms) Space</kbd>, <kbd>M1 (10ms delay) Space</kbd>, and <kbd>M1 &gt; 10ms &gt; Space</kbd> press M1, wait exactly 10 ms, then show Space. Use “Wait” for an empty beat.</p>
            </div>}
        ...
        </div> : <div className="editor-empty"><span>＋</span><h1>Add the first tech</h1><p>Techs become the light-blue list for this class.</p><button onClick={addTech}>Add tech</button></div>}
      </section>
    </div>
